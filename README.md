...
This [article](retrieval_based_chatbot_MASTER1.md) discusses the role of vector databases in domain-specific chatbots.  Companies can use such chatbots to leverage ChatGPT's extraordinary natural language capabilities while limiting answers to company-provided documents and information. This specificity is achieved using an approach called retrieval-based augmentation. The article covers integration with Pinecone, a cloud-based vector database provider.  It includes code excerpted from an end-to-end Jupyter [notebook]() **UPDATE LINK**.  See the next two items below for other examples of customized chatbots (without vector database integration).  

...
This [article]() **UPDATE LINK** covers the central role vector databases play in scaling domain-specific chatbots (see earlier examples of such chatbots below and [here](https://github.com/robjm16/domain_specific_ChatGPT)).  With these  chatbots, companies can leverage ChatGPT's extraordinary natural language capabilities while limiting its answers to company-specific documents and information. Tailoring ChatGPT is achieved via an approach called retrieval-based augmentation. My article covers this approach and includes code snippets from a related end-to-end [Jupyter notebook]()**UPDATE**.  The notebook covers integration with Pinecone, a cloud-based vector database provider. 
